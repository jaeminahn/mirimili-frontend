...
export default function SearchResultsContent({ query }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const url = `/posts/search?q=${encodeURIComponent(
        query
      )}&page=${page}&size=10`;
      const res = await getJSON<ApiEnvelope<PageResult<SearchPost>>>(url);
      ...
      if (res.ok && res.data?.success) {
        ...
        setErrMsg(
          res.data?.message || "검색 결과를 불러오는 중 오류가 발생했습니다."
        );
        setPosts([]);
        setTotalPages(0);
        setHasNext(false);
  ...
  const renderPosts = () => {
    if (!query || !query.trim())
      return <p className="text-gray-600">검색어를 입력해주세요.</p>;
    if (loading) return <p className="text-gray-500">불러오는 중…</p>;
    if (errMsg) return <p className="text-red-600">{errMsg}</p>;
    if (!posts.length)
      return <p className="text-gray-600">검색 결과가 없습니다.</p>;
