...
export default function SetTypeAndStartDate({
  ...
  return (
    <div className="flex flex-col p-6 bg-white rounded-3xl w-[360px]">
      <h2 className="mb-4 text-lg font-bold">복무 형태 설정</h2>
      ...
      <h2 className="mb-4 text-lg font-bold">입대일</h2>
      <DatePicker date={startDate} setDate={setStartDate} />

      <div className="flex items-center gap-4 p-2 px-4 mt-4 mb-2 text-sm rounded-lg bg-emerald-100 text-emerald-600">
        <Icon icon="fluent:info-28-filled" className="text-2xl" />
        타군 대상으로도 서비스 확대 예정이에요
      </div>
      <div className="flex items-center gap-4 p-2 px-4 text-sm rounded-lg bg-red-100 text-red-600">
        <Icon icon="fluent:warning-28-filled" className="text-3xl" />
        지금 입력하시는 정보는 추후 변경이 어려우니 신중하게 설정해주세요
      </div>
    </div>
  );
}
